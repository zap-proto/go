...
func EmitCPP(f *idl.File) (map[string][]byte, error) {
	...
	for _, iface := range f.Interfaces {
		...
		var w bytes.Buffer
		writeCPPPrologue(&w, f, cppNeeds(f, nil, []*idl.Interface{iface}))
		writeCPPOpen(&w, f)
...
func EmitCPPSingle(f *idl.File) (string, []byte, error) {
...
func cppNeeds(file *idl.File, structs []*idl.Struct, ifaces []*idl.Interface) needs {
	var n needs
	...
	for _, s := range structs {
		...
		for _, f := range s.Fields {
			...
			case idl.KindList:
				...
				if file.Shape(elem) == idl.ShapeInline {
					n.fixed = true
					n.mem = true
				}
			}
...
func writeCPPPrologue(w *bytes.Buffer, f *idl.File, n needs) {
...
// already in the file.
func writeCPPIncludes(w *bytes.Buffer, f *idl.File, s *idl.Struct) {
...
func cppNamespace(f *idl.File) string {
	return strings.ReplaceAll(f.Package, ".", "::")
}

func writeCPPOpen(w *bytes.Buffer, f *idl.File) {
	fmt.Fprintf(w, "\nnamespace %s {\n\n", cppNamespace(f))
}

func writeCPPClose(w *bytes.Buffer, f *idl.File) {
...
func writeCPPForwards(w *bytes.Buffer, file *idl.File, structs []*idl.Struct) {
...
func emitCPPStruct(w *bytes.Buffer, f *idl.File, s *idl.Struct) {
	emitCPPOffsets(w, s)
	emitCPPReader(w, f, s)
}
...
func cppTypeName(f *idl.File, name string) string {
	return "::" + cppNamespace(f) + "::" + name
}

func emitCPPOffsets(w *bytes.Buffer, s *idl.Struct) {
	...
	fmt.Fprintf(w, "inline constexpr std::int64_t %s = %d;\n\n", cppSizeName(s), idl.StructSize(s))
}

func cppOffsetName(s *idl.Struct, f *idl.Field) string { return "k" + s.Name + f.Name + "Off" }
func cppSizeName(s *idl.Struct) string                 { return "k" + s.Name + "Size" }

func emitCPPReader(w *bytes.Buffer, f *idl.File, s *idl.Struct) {
...
func emitCPPFieldReader(w *bytes.Buffer, file *idl.File, s *idl.Struct, f *idl.Field) {
...
func emitCPPOutOfLine(w *bytes.Buffer, file *idl.File, structs []*idl.Struct) {
	first := true
	head := func() {
		if first {
	...
	for _, s := range structs {
		for _, f := range s.Fields {
			...
			case idl.KindList:
				...
				if shape != idl.ShapeInline && shape != idl.ShapePointer {
					continue
				}
				head()
				name := cppTypeName(file, elem.StructName)
				reach := fmt.Sprintf("object_ptr(i)")
				if shape == idl.ShapeInline {
...
func emitCPPInput(w *bytes.Buffer, file *idl.File, s *idl.Struct) {
...
func emitCPPWriterDecls(w *bytes.Buffer, file *idl.File, structs []*idl.Struct) {
	if len(structs) == 0 {
		return
	}
...
func cppArraySize(s *idl.Struct) string {
	return fmt.Sprintf("static_cast<std::size_t>(%s)", cppSizeName(s))
}

func emitCPPWriter(w *bytes.Buffer, file *idl.File, s *idl.Struct) {
	if !file.Tail(s) {
...
func emitCPPEncode(w *bytes.Buffer, file *idl.File, s *idl.Struct) {
...
func emitCPPEncodeField(w *bytes.Buffer, s *idl.Struct, f *idl.Field) {
...
func emitCPPAppend(w *bytes.Buffer, file *idl.File, s *idl.Struct) {
...
func emitCPPNew(w *bytes.Buffer, file *idl.File, s *idl.Struct) {
...
func emitCPPBody(w *bytes.Buffer, file *idl.File, s *idl.Struct) {
...
func cppPayloadVar(f *idl.Field) string { return lowerFirst(f.Name) + "_off" }
...
func emitCPPPayload(w *bytes.Buffer, file *idl.File, s *idl.Struct, f *idl.Field, p pass) {
	switch f.Type.Kind {
	case idl.KindStruct:
		if p != payloadObjects {
			return
...
func cppListAdd(elem idl.Type) string {
...
func cppListValue(elem idl.Type) string {
	switch elem.Kind {
	case idl.KindBool:
		return "elem ? 1 : 0"
	case idl.KindI8:
...
func cppInputType(file *idl.File, t idl.Type) string {
	switch t.Kind {
	case idl.KindBool:
		return "bool"
	case idl.KindU8:
...
func emitCPPFieldWriter(w *bytes.Buffer, file *idl.File, s *idl.Struct, f *idl.Field) {
	off := cppOffsetName(s, f)
	switch f.Type.Kind {
	case idl.KindBool:
...
func emitCPPInterface(w *bytes.Buffer, iface *idl.Interface) {
	emitCPPOrdinals(w, iface)
	emitCPPClient(w, iface)
	emitCPPServer(w, iface)
}

func emitCPPOrdinals(w *bytes.Buffer, iface *idl.Interface) {
...
func emitCPPClient(w *bytes.Buffer, iface *idl.Interface) {
...
func cppAnswer(iface *idl.Interface, m *idl.Method) string {
	if m.Response != nil {
...
func emitCPPClientMethod(w *bytes.Buffer, iface *idl.Interface, m *idl.Method) {
...
func emitCPPClientInvoke(w *bytes.Buffer, iface *idl.Interface, m *idl.Method) {
	mname := exportIdent(m.Name)
...
func cppMethodArg(m *idl.Method) (arg, payload string) {
	if m.Request == nil {
		return "", "{}"
	}
...
func emitCPPServer(w *bytes.Buffer, iface *idl.Interface) {
...
func emitCPPDispatchCase(w *bytes.Buffer, m *idl.Method) {
	mname := exportIdent(m.Name)
	callArg := ""
	if m.Request != nil {
...
func named(file *idl.File, s *idl.Struct) []string {
	var out []string
	for _, f := range s.Fields {
		switch f.Type.Kind {
		case idl.KindStruct:
		...
		case idl.KindList:
			if f.Type.ListElem.Kind == idl.KindStruct {
...
func emitCPPWriters(w *bytes.Buffer, file *idl.File, structs []*idl.Struct) error {
	if len(structs) == 0 {
		return nil
	}
...
func inputOrder(file *idl.File, structs []*idl.Struct) ([]*idl.Struct, error) {
	const (
		unseen = 0
		open   = 1
		done   = 2
	...
	var out []*idl.Struct
	var visit func(s *idl.Struct) error
	visit = func(s *idl.Struct) error {
		if !here[s.Name] {
...
// the caller's hands.
func emitCPPElement(w *bytes.Buffer, file *idl.File, f *idl.Field, elem idl.Type) {
	switch file.Shape(elem) {
	case idl.ShapeFixed:
