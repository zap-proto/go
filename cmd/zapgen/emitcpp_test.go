...
func TestUnknownLangIsRefused(t *testing.T) {
	if _, _, err := backend("rust"); err == nil {
		t.Error("backend(\"rust\") should fail until a Rust emitter exists")
	}
	for _, lang := range []string{"go", "cpp"} {
		if _, _, err := backend(lang); err != nil {
			t.Errorf("backend(%q): %v", lang, err)
		}
	}
}
