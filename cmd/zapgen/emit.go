...
func EmitSingle(f *File) (string, []byte, error) {
	...
	var w bytes.Buffer
	writeFileHeader(&w, f.Package, source, f.Structs, len(f.Interfaces) > 0)
...
func writeFileHeader(w *bytes.Buffer, pkg, source string, structs []*Struct, needsRPC bool) {
	// A record packer writes its scalars itself, which is the one thing in
	// the emitted file that reaches past the runtime.
	binaryNeeded, mathNeeded := false, false
	for _, s := range structs {
		...
		for _, f := range s.Fields {
			...
			case KindF32, KindF64:
				binaryNeeded, mathNeeded = true, true
			}
		}
	}
...
func emitStruct(f *File, s *Struct) ([]byte, error) {
	...
	var w bytes.Buffer
	writeFileHeader(&w, f.Package, sourceName(f), []*Struct{s}, false)
	emitOffsets(&w, s)
	emitReader(&w, f, s)
	emitBuilder(&w, f, s)
	return w.Bytes(), nil
...
func emitInterfaceFile(f *File, iface *Interface) ([]byte, error) {
	...
	var w bytes.Buffer
	writeFileHeader(&w, f.Package, sourceName(f), nil, true)
	emitInterface(&w, iface)
	return w.Bytes(), nil
}
	...
	for _, f := range s.Fields {
		...
		if f.Type.Kind == KindPtr {
			return fmt.Errorf("struct %s field %s: ptr<%s> names one element of a list; "+
				...
				s.Name, f.Name, f.Type.StructName, f.Name, f.Type.StructName)
		}
		sz := f.Type.SlotSize()
		if sz == 0 {
...
func emitReader(w *bytes.Buffer, f *File, s *Struct) {
...
// emitElemReader emits the typed element accessor of a list of structs, so a
// caller reads one element by name instead of by stride. An inline element
...
func emitBuilder(w *bytes.Buffer, f *File, s *Struct) {
	// Input struct.
	fmt.Fprintf(w, "\n// %sInput collects the field values for New%s.\n", s.Name, s.Name)
	fmt.Fprintf(w, "type %sInput struct {\n", s.Name)
	for _, fld := range s.Fields {
		fmt.Fprintf(w, "\t%s %s\n", fld.Name, goInputType(f, fld.Type))
	}
	w.WriteString("}\n\n")
	...
	lower := lowerFirst(s.Name)
	fmt.Fprintf(w, "// Put%s writes a %s into b and returns where its object landed.\n", s.Name, s.Name)
	w.WriteString("//\n")
	w.WriteString("// What a field points AT is written first, in field order, and the fixed\n")
	w.WriteString("// section last: a pointer always leads backward, to bytes already placed.\n")
	fmt.Fprintf(w, "func Put%s(b *zap.Builder, in %sInput) int {\n", s.Name, s.Name)
	for _, fld := range s.Fields {
		emitLeaf(w, f, fld)
	}
	for _, fld := range s.Fields {
	...
	fmt.Fprintf(w, "\tb.SetRoot(Put%s(b, in))\n", s.Name)
	w.WriteString("\treturn b.Finish()\n")
	w.WriteString("}\n")
}

// emitPack emits the record form of an inline struct: its Size bytes, which
// is what one element of a list of it looks like on the wire. A struct with a
// tail has no record form — it is written as its own message.
func emitPack(w *bytes.Buffer, s *Struct) {
	if !s.Inline() {
		return
	}
	lower := lowerFirst(s.Name)
	fmt.Fprintf(w, "// Pack%s returns %s as the %sSize bytes one element of a list holds.\n",
		s.Name, s.Name, lower)
	fmt.Fprintf(w, "func Pack%s(in %sInput) [%sSize]byte {\n", s.Name, s.Name, lower)
	fmt.Fprintf(w, "\tvar r [%sSize]byte\n", lower)
	for _, fld := range s.Fields {
		off := fmt.Sprintf("%s%sOff", lower, fld.Name)
		switch fld.Type.Kind {
		case KindBool:
			fmt.Fprintf(w, "\tif in.%s {\n\t\tr[%s] = 1\n\t}\n", fld.Name, off)
		case KindU8:
			fmt.Fprintf(w, "\tr[%s] = in.%s\n", off, fld.Name)
		case KindI8:
			fmt.Fprintf(w, "\tr[%s] = uint8(in.%s)\n", off, fld.Name)
		case KindU16:
			fmt.Fprintf(w, "\tbinary.LittleEndian.PutUint16(r[%s:], in.%s)\n", off, fld.Name)
		case KindI16:
			fmt.Fprintf(w, "\tbinary.LittleEndian.PutUint16(r[%s:], uint16(in.%s))\n", off, fld.Name)
		case KindU32:
			fmt.Fprintf(w, "\tbinary.LittleEndian.PutUint32(r[%s:], in.%s)\n", off, fld.Name)
		case KindI32:
			fmt.Fprintf(w, "\tbinary.LittleEndian.PutUint32(r[%s:], uint32(in.%s))\n", off, fld.Name)
		case KindF32:
			fmt.Fprintf(w, "\tbinary.LittleEndian.PutUint32(r[%s:], math.Float32bits(in.%s))\n", off, fld.Name)
		case KindU64:
			fmt.Fprintf(w, "\tbinary.LittleEndian.PutUint64(r[%s:], in.%s)\n", off, fld.Name)
		case KindI64:
			fmt.Fprintf(w, "\tbinary.LittleEndian.PutUint64(r[%s:], uint64(in.%s))\n", off, fld.Name)
		case KindF64:
			fmt.Fprintf(w, "\tbinary.LittleEndian.PutUint64(r[%s:], math.Float64bits(in.%s))\n", off, fld.Name)
		case KindBytesFixed:
			fmt.Fprintf(w, "\tcopy(r[%s:], in.%s[:])\n", off, fld.Name)
		}
	}
...
// emitLeaf writes the objects a field's pointers will name, before any
// pointer run exists. Every one of them is written first, in field order,
// which is where the reference wire puts them.
func emitLeaf(w *bytes.Buffer, f *File, fld *Field) {
	switch fld.Type.Kind {
	case KindStruct:
		...
		fmt.Fprintf(w, "\tat%s := b.Embed(in.%s)\n", fld.Name, fld.Name)
	case KindList:
		elem := f.PtrElem(fld.Type)
		if elem == nil {
			return
		}
		fmt.Fprintf(w, "\toffs%s := make([]int, 0, len(in.%s))\n", fld.Name, fld.Name)
		fmt.Fprintf(w, "\tfor _, elem := range in.%s {\n", fld.Name)
		fmt.Fprintf(w, "\t\toffs%s = append(offs%s, Put%s(b, elem))\n", fld.Name, fld.Name, elem.Name)
		w.WriteString("\t}\n")
	}
}

// emitList lays down one list's run of elements: records end to end for an
// inline element, a length before each entry for one with a tail, and a
// signed offset per element for a list of pointers.
func emitList(w *bytes.Buffer, f *File, fld *Field) {
	if fld.Type.Kind != KindList {
		return
	}
	listVar := lowerFirst(fld.Name) + "LB"
...
func goInputType(f *File, t Type) string {
	switch t.Kind {
	case KindBool:
		return "bool"
	case KindU8:
...
func emitFieldWriter(w *bytes.Buffer, lower string, f *Field) {
	...
	case KindList:
		fmt.Fprintf(w, "\tob.SetList(%s, at%s, len(in.%s))\n", offsetConst, f.Name, f.Name)
	case KindStruct:
		fmt.Fprintf(w, "\tob.SetObject(%s, at%s)\n", offsetConst, f.Name)
	}
}
