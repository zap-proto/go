...
func EmitSingle(f *idl.File) (string, []byte, error) {
	...
	var w bytes.Buffer
	writeFileHeader(&w, f.Package, source, f.Doc, len(f.Structs) > 0, len(f.Interfaces) > 0)
...
func writeFileHeader(w *bytes.Buffer, pkg, source string, doc []string, needsZap, needsRPC bool) {
...
// writeDoc writes the schema's own words above a generated declaration.
//
// This is the whole point of carrying comments through the parser: the
// sentence a reader sees on the accessor is the sentence the schema author
// wrote next to the field, not a second description maintained somewhere
// else that drifts from it. Where the schema says nothing, the generator's
// own derived line stands — an empty doc emits nothing, so silence in the
// schema is silence here.
func writeDoc(w *bytes.Buffer, doc []string) bool {
...
func emitStruct(f *idl.File, s *idl.Struct) ([]byte, error) {
	...
	var w bytes.Buffer
	writeFileHeader(&w, f.Package, sourceName(f), f.Doc, true, false)
	emitOffsets(&w, s)
	emitReader(&w, s)
	emitBuilder(&w, s)
	return w.Bytes(), nil
...
func emitInterfaceFile(f *idl.File, iface *idl.Interface) ([]byte, error) {
	...
	var w bytes.Buffer
	writeFileHeader(&w, f.Package, sourceName(f), f.Doc, false, true)
	emitInterface(&w, iface)
	return w.Bytes(), nil
}
