...
func writeFileHeader(w *bytes.Buffer, file *File, pkg, source string, structs []*Struct, needsRPC bool) {
...
func goNeeds(file *File, structs []*Struct) goNeed {
	var n goNeed
	for _, s := range structs {
		inline := !file.Tail(s)
		for _, f := range s.Fields {
			switch f.Type.Kind {
			case KindU16, KindU32, KindU64, KindI16, KindI32, KindI64:
			...
			case KindList:
				...
				if elem.Kind == KindF32 || elem.Kind == KindF64 {
					n.floats = true
				}
			}
		}
...
// structSize is the width of the fixed section: what the schema says, or
// where the last field ends when the schema says nothing. Nothing is rounded
// — the layout is the author's, and a record whose reserved width is wider
// than its fields says so with `struct Name @N`.
...
func emitStruct(f *File, s *Struct) ([]byte, error) {
	...
	var w bytes.Buffer
	writeFileHeader(&w, f, f.Package, sourceName(f), []*Struct{s}, false)
	emitOffsets(&w, s)
	emitReader(&w, f, s)
	emitBuilder(&w, f, s)
	return w.Bytes(), nil
...
func validate(file *File, s *Struct) error {
	size := structSize(s)
	if size == 0 {
...
func emitReader(w *bytes.Buffer, file *File, s *Struct) {
...
func emitBuilder(w *bytes.Buffer, file *File, s *Struct) {
	emitInput(w, file, s)
	if !file.Tail(s) {
		emitEncode(w, s)
	}
...
func emitInput(w *bytes.Buffer, file *File, s *Struct) {
...
func emitEncode(w *bytes.Buffer, s *Struct) {
	lower := lowerFirst(s.Name)
...
func emitEncodeField(w *bytes.Buffer, lower string, f *Field) {
...
func emitAppend(w *bytes.Buffer, file *File, s *Struct) {
...
func emitNew(w *bytes.Buffer, file *File, s *Struct) {
...
func emitBody(w *bytes.Buffer, file *File, s *Struct) {
	lower := lowerFirst(s.Name)
...
func payloadVar(f *Field) string { return lowerFirst(f.Name) + "Off" }
...
// byte after it.
func emitPayload(w *bytes.Buffer, file *File, lower string, f *Field, p pass) {
	switch f.Type.Kind {
	case KindStruct:
		if p != payloadObjects {
			return
...
func goListAdd(elem Type) string {
...
func goListValue(elem Type) string {
	switch elem.Kind {
	case KindI8:
		return "uint8(elem)"
	case KindI32:
...
func validateField(file *File, s *Struct, f *Field) error {
	switch f.Type.Kind {
	case KindStruct:
...
// the caller's hands.
func emitElement(w *bytes.Buffer, file *File, structName string, f *Field, elem Type) {
	switch file.Shape(elem) {
	case ShapeFixed:
